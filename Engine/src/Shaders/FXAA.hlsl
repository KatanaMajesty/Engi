...
Texture2D<float4> g_ldrBuffer : register(t0); // .rgb = LDR sRGB color (after gamma-correction), .a = color luma

cbuffer FXAASpecification : register(b0)
{
...
float4 ps_main(VS_OUTPUT input) : SV_Target0
{
    ...
    TextureAndSampler.tex = g_ldrBuffer;
    TextureAndSampler.smpl = g_samplerLinearClamp;
